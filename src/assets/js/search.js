/*!
 * JavaScript for the Bootstrap Blog search (https://blog.getbootstrap.com/)
...
const RECENT_STORAGE_KEY = 'bd:blog:search:recent'
const RECENT_LIMIT = 5

const instance = getInstanceManager().getInstance('default')
let searchInitialized = false

// The Dialog component lives on the globally-loaded `bootstrap.bundle.min.js`,
// which is also what powers the trigger's `data-bs-toggle="dialog"`. Reusing
// that copy keeps a single instance per dialog element.
const getDialog = () => globalThis.bootstrap?.Dialog
...
const registerResultLinkHandler = () => {
  ...
  document.addEventListener('click', event => {
    ...
    getDialog()?.getInstance(dialogEl)?.hide()
  })
}
