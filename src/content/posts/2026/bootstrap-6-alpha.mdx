...
{/*
  ...
  This is an .mdx post so it can render live examples. Example,
  ResizableExample, and ButtonPlayground are ported from the docs and live in
  src/components/shortcodes/. All the `example:` placeholders are now real
  demos.

  Remaining follow-ups, each marked inline with a TODO comment:
    - TODO(screenshot) x5 — captures that need a full page, a real device, or
      motion, so they can't be live embeds. Find them with:
        rg 'TODO\(screenshot\)' src/content/posts/2026/bootstrap-6-alpha.mdx
    - TODO(grid) — the Grid section is stubbed pending grid system decisions.
    - TODO(form-adorn) — needs a source decision before it earns a mention.

  Still worth porting if we want it: ScssDocs, which prints the source between
  `// scss-docs-start <name>` and `// scss-docs-end <name>` markers so snippets
  stay in sync with node_modules/bootstrap/scss/.
*/}
